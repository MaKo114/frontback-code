...
const NavbarAdmin = () => {
  ...
  const fetchPendingCount = async () => {
    try {
      ...
      const count = allReports.filter((r: any) => r.status === "PENDING").length;
  ...
  }, [token]); // ให้ทำงานเมื่อโหลดหน้าหรือ token เปลี่ยน
  ...
  return (
    <aside className="w-72 h-screen sticky top-0 bg-white flex flex-col border-r border-gray-200 shadow-sm font-['Inter',_sans-serif]">
      ...
      <nav className="flex-1 px-4 py-6 space-y-1.5 overflow-y-auto">
        <p className="px-4 text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Main Menu</p>
        {menuItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              <span className={`font-semibold ${isActive ? "font-bold" : ""}`}>{item.name}</span>
              
              {/* แสดงตัวเลขเฉพาะเมนูรายงาน และถ้ามีจำนวนมากกว่า 0 */}
              {item.name === "รายงาน" && pendingCount > 0 && (
                <span className={`ml-auto px-2 py-0.5 text-[10px] rounded-full font-black min-w-[20px] text-center ${
                  isActive ? 'bg-white text-[#FF5800]' : 'bg-red-500 text-white shadow-sm'
                }`}>
                  {pendingCount}
                </span>
              )}
      ...
      <div className="p-6 border-t border-gray-100 flex-shrink-0 bg-gray-50/50">
        <div className="flex items-center gap-3 mb-4 px-1">
          ...
          <div className="flex-1 min-w-0 text-left">
            ...
            <p className="text-[#FF5800] text-[10px] font-bold truncate">{email}</p>
          </div>
        </div>
        <button 

...
export const ownerConfirmExchange = async ({ params, user, set }: any) => {
  try {
    if (!user?.student_id) {
      set.status = 401;
      return { error: "Please login first" };
    ...
    const result = await exchangeService.ownerConfirm(exchange_id, user.student_id);
    set.status = 200;
    return { message: "Owner confirmed", data: result };
  } catch (err: any) {
    set.status = 400;
    return { error: err.message };
  }
...
export const requesterConfirmExchange = async ({ params, user, set }: any) => {
  try {
    if (!user?.student_id) {
      set.status = 401;
      return { error: "Please login first" };
    ...
    const result = await exchangeService.requesterConfirm(exchange_id, user.student_id);
    set.status = 200;
    return { message: "Requester confirmed", data: result };
  } catch (err: any) {
    set.status = 400;
    return { error: err.message };
  }
